...
void printSentenceMenu() {
    ...
    printf(" 2. Сделать предложением пустым\n");
...
void printSequenceMenu() {
    ...
    printf(" 3. Проверить последовательность пуста/непуста\n");
...
SeqList* chooseSequenceAction(SeqList* seq) {
    ...
    while (loop) {
        ...
        if (seqWorkState == false && a > 1 && a < 13) {
...
int main() {
    ...
    //SentList sentList = { NULL, NULL };
    //SeqList seqList = { NULL, NULL, NULL };
    //chooseSentenceAction(&sentList);
